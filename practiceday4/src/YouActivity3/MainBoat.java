...
public class MainBoat {
	...
	public static void main(String[] args) {
		...
		sb.setCapacity(3);
		// Create 10 objects of the Passenger class one by one and take input also
		int counter = 1;
		Passenger ps = null;
		do {
			...
			if (choice == 1) {
				...
				if (isAlloted == true) {
					...
					System.out.println("No motor boat available. Wait for the next round.");
				}
			} else if (choice == 2) {
				...
				if (isAlloted == true) {
					...
					System.out.println("No speed boat available. Wait for the next round.");
				}
			}
			System.out.println();
		} while (++counter <= 10);
